...
function fail($msg) {
	...
	$uploadResult[1] = $msg;
	sendResponse($uploadResult, $config['BaseUrl']);
	die;
}
...
function run() {
	...
	if (!isset($config)) {
		...
		if (isset($_GET['iw']))
			$imgWidth = $_GET['iw'];
		if (preg_match('/^\d{1,5}$/', $imgWidth) === FALSE)
		...
		if (isset($_GET['ih']))
			$imgHeight = $_GET['ih'];
		if (preg_match('/^\d{1,5}$/', $imgHeight) === FALSE)
		...
		if (isset($_GET['tw']))
			$thumbWidth = $_GET['tw'];
		if (preg_match('/^\d{1,5}$/', $thumbWidth) === FALSE)
		...
		if (isset($_GET['th']))
			$thumbHeight = $_GET['th'];
		if (preg_match('/^\d{1,5}$/', $thumbHeight) === FALSE)
